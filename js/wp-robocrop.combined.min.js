...
a.url+'"]').each(function(){h(this).attr("src",a.url+"?"+b.getTime())})},d);c.prop("disabled",!1)});return this},cancel:function(){k?p.content.mode(k):m&&p.setState(m);return this},onselecttool:function(){var a=this.selectRatio.getSelected();this.$areaSelect().cancelSelection();switch(a){case "focuspoint":this.focuspointtool.setEnabled(!0)}},onselectratio:function(){this.focuspointtool.setEnabled(!1);var a=this.selectRatio.getSelected(),b=this.model.get("sizes"),c,d,f=this,l,h=this.model.get("width"),
k=this.model.get("height");this.current_ratio=this.image_ratios[a];l={aspectRatio:this.current_ratio.ratio+":1",minWidth:this.current_ratio.min_width,minHeight:this.current_ratio.min_height};_.each(this.current_ratio.sizes,function(a){!d&&b[a]&&b[a].cropdata&&(d=b[a].cropdata);g[a].width<=h&&g[a].height<=k&&(l.minWidth=Math.max(l.minWidth,g[a].width),l.minHeight=Math.max(l.minHeight,g[a].height))});d?(c={},_.extend(c,d)):(a=Math.min(this.model.get("width")/this.current_ratio.ratio,this.model.get("height")),
c={x:0,y:0,width:a*this.current_ratio.ratio,height:a},c.x=(this.model.get("width")-c.width)/2,c.y=(this.model.get("height")-c.height)/2);this.$areaSelect().setOptions(l);if(this.image.$el.get(0).complete)this.selectCrop(c);else this.image.$el.on("load",function(){f.selectCrop(c)});return this},autocrop:function(a){var e={width:this.model.get("width"),height:this.model.get("height"),focuspoint:this.model.get("focuspoint")};a=b.media.robocrop.cropFromFocusPoint(e,this.current_ratio);a=b.media.robocrop.relToAbsCoords(a,
e);this._setCropSizes(a);this.selectCrop(a);return this},selectCrop:function(a){this._image_scale_factor();a=b.media.robocrop.rectToPointCoords(a);var e=this.$areaSelect();e.setSelection(a.x1,a.y1,a.x2,a.y2,!1);e.setOptions({show:!0});e.update();return this},$areaSelect:function(){return this.image.$el.data("imgAreaSelect")},_image_scale_factor:function(){var a=this.image.$el.closest(".robocrop-image-box"),b=Math.min(this.model.get("width"),a.width()),a=Math.min(this.model.get("height"),a.height());
